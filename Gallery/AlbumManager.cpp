...
bool isTriggerd = false;  // use for the process
...
void AlbumManager::showPicture()
{
	...
	SetConsoleCtrlHandler(consoleHandler, TRUE);  // Make sure we use the ctrl c handler
	MenuPictures(pic);  // open pic
}
...
void AlbumManager::removeUser()
{
	...
	//Remove all the albums of the user
	auto albums = m_dataAccess.getAlbumsOfUser(user);
	while (!albums.empty())
	{
		m_dataAccess.deleteAlbum(albums.front().getName(), user.getId());
		albums.pop_front();
	}

	//Remove all the places the user was tagged in
	for (const auto& album : m_dataAccess.getAlbums()) {
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Count of Albums as Owner: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl;
}
...
//This function gives the option to open picture with 2 diffrend apps
void AlbumManager::MenuPictures(Picture picture)
{
	std::string openWith = "";
	std::string fullCommand = "";
	int choice = 0;
	do //Get the choice of the user
	{
		std::cout << "Press 1 to open picture with \"paint\"." << std::endl;
		std::cout << "Press 2 to open picture with \"IrfanView\"." << std::endl;
		std::cin >> choice;
		std::cin.clear(); // clean buffer
	} while (choice != PAINT && choice != IRFAN_VIEW);
	switch (choice)  // switch case if we would like to add more in the future
	{
	case PAINT:
		openWith = "mspaint.exe";
		break;
	case IRFAN_VIEW:
		openWith = "\"C:\\Program Files\\IrfanView\\i_view64.exe\"";
		break;
	default:
		break;
	}

	fullCommand = openWith + " " + picture.getPath() + "";  // prepare for execution

	//Start creating the proccesses
	STARTUPINFO si;
	...
	if (!CreateProcess(NULL, (LPSTR)fullCommand.c_str(), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
	{
	...
	while (!isTriggerd);
	isTriggerd = false;
	TerminateProcess(pi.hProcess, 9);  // kill the process
...
//This function is for telling when pressed ctrl+c
BOOL __stdcall AlbumManager::consoleHandler(DWORD signal)
{
